# 引き継ぎ文：呉服Quiz工房

## このドキュメントの位置づけ

前回のセッション（SeriesGuide スキーマ改訂）の成果物です。
**アプリは実装済み。`feature/beyond-mvp` ブランチで作業中。**

### AIとの共同作業の範囲
- **共同**: スキル作成・動作確認・バグ修正・UI改善・プロンプト調整・Quiz工房のブラッシュアップ
- **制作者が単独で行う**: ガイドの中身を書く・画面キャプチャ・図解作成・提出

**次のチャットは「シリーズガイドの作成」から始めてください。**
その前に `pnpm seed`（DB を最新 JSON に合わせる）→ `pnpm dev` の順で起動してください。
新しいチャットで「シリーズガイドを作る」と話しかけると
`create-series-guide` スキルが自動で呼び出されます。

---

## プロジェクト概要

| 項目 | 内容 |
|------|------|
| システム名 | 呉服Quiz工房（制作スタジオ） |
| 成果物の名称 | 呉服販売員ドリル |
| 制作者 | 百貨店呉服部の部門長（本人のみ） |
| 期限 | 2026年8月28日 |
| 提出物 | 図解のURL（スクリーンショット＋説明。アプリのURLは不要） |

---

## 技術スタック

- **フレームワーク**: Next.js 16 + Tailwind CSS 4
- **AI**: Anthropic Claude API（`@anthropic-ai/sdk`）。モデル: `claude-opus-4-5`
- **起動**: `pnpm dev`（ワークスペースルートで実行）→ http://localhost:3000
- **APIキー**: 画面の設定欄（⚙）に貼るだけ。ファイルに書かない・保存されない
- **データ**: SQLite（`data/app.db`）+ Drizzle ORM で管理。`pnpm seed` で初期データ投入

---

## ファイル構成

```
Graduation-work/
├── app/
│   ├── api/
│   │   ├── data/route.ts           GET  全データ読み込み
│   │   ├── save-guide/route.ts     POST シリーズ/コースガイド保存
│   │   ├── generate/route.ts       POST Claude API でレッスン生成
│   │   ├── save-lesson/route.ts    POST 設問保存（検査も再実行）
│   │   └── approve-lesson/route.ts POST「これでよい」承認
│   ├── preview/[lessonId]/page.tsx プレビュー（1問ずつ解く）
│   ├── globals.css
│   ├── layout.tsx
│   └── page.tsx                    3ペインメイン画面
├── data/
│   ├── app.db                      SQLiteデータベース本体（git管理外）
│   └── kakuto-tpo/                 シード元JSONファイル（参照用に残す）
│       ├── series.json             シリーズガイド
│       └── kimono-kaku/
│           ├── course.json         コースガイド
│           └── lessons/
│               ├── lesson-01.json  第一礼装（status: approved）
│               ├── lesson-02.json  準礼装・略礼服（status: pending）
│               └── lesson-03.json  しゃれもの（status: pending）
├── lib/
│   ├── types.ts                    型定義
│   ├── data.ts                     Drizzle クエリヘルパー（API公開関数）
│   ├── inspect.ts                  機械検査ロジック
│   ├── prompt.ts                   Claude プロンプト構築
│   └── db/
│       ├── schema.ts               Drizzle スキーマ定義（4テーブル）
│       └── index.ts                DB接続シングルトン（better-sqlite3）
├── scripts/
│   └── seed.ts                     JSON→SQLite 初期データ移行（pnpm seed）
├── .cursor/
│   └── skills/
│       ├── quiz-studio-reference/        Quiz Studio参照スキル
│       ├── create-lesson-questions/      レッスン問題作成スキル
│       ├── create-series-guide/          シリーズガイド作成（grill-me → Sonnet → 保存）
│       ├── create-course-guide/          コースガイド作成（grill-me → Sonnet → 保存）
│       ├── review-series-guide/          シリーズガイドレビュー（6観点 → 改善 → 保存）
│       └── review-course-guide/          コースガイドレビュー（シリーズ照合 → 保存）
├── drizzle.config.ts               Drizzle Kit 設定
├── HANDOFF.md                      このファイル
└── package.json
```

---

## コンテンツの4階層

```
呉服販売員ドリル（総称）
└ シリーズ  格とTPO
   ├ シリーズガイド（人間が先に書く）
   └ コース1  着物の格
      ├ コースガイド（人間が先に書く）
      ├ レッスン1  第一礼装
      ├ レッスン2  準礼装・略礼服
      └ レッスン3  しゃれもの

各レッスン
├ 状態: 未生成 / 下書き / これでよい
├ 検査: 未実施 / 合格 / 不合格（理由）
├ 生成エラー（あれば下書きは触らない）
└ 設問 × 8
   ├ [四択]  問題文 / 選択肢4 / 正解インデックス / 解説
   └ [○×]   問題文 / 正解（true/false）/ 解説
   （手順タイプは後日追加。MVPには含まない）
```

---

## UIの画面構成（3ペイン）

```
┌──────────────┬────────────────────────┬────────────────────────┐
│ P1 ツリー    │ P2 ガイド              │ P3 編集 / 設問          │
│ （左サイド） │ （中央）               │ （右メイン）            │
│──────────────│────────────────────────│────────────────────────│
│▼ 格とTPO    │← シリーズ選択:        │← レッスン選択:        │
│  ▼ 着物の格  │  シリーズガイド編集    │  設問一覧              │
│   □ レッスン1│                        │  [生成] [再生成]       │
│   □ レッスン2│← コース選択:          │  検査: 合格 / 不合格   │
│   □ レッスン3│  コースガイド編集      │  [これでよい]          │
│              │                        │  [プレビュー]          │
│[⚙設定]      │← レッスン/設問選択:   │← 設問選択: 1問編集     │
│              │  コースガイドを参照表示│                        │
└──────────────┴────────────────────────┴────────────────────────┘
凡例: □未生成  ○下書き  ●これでよい
```

P4（AIチャット壁打ち）は **後日追加**。MVPには含まない。

---

## 6工程（レッスン1本を作る手順）

1. シリーズガイド＋コースガイドを書く（P1でシリーズ/コースを選択 → P2で記入 → [保存]）
2. レッスンを選んで [このレッスンを生成]（APIキーが必要）
3. プログラムが形式を自動検査する（8問・四択/○×形式・解説・禁止語）
4. 悪い設問だけ P1 でクリック → P3 で手直し
5. 検査合格後に [これでよい ✓]（次のレッスン生成の参照元になる）
6. [プレビュー ▶] で8問を解く（検品用。点数は保存されない）

---

## 機械検査の内容

| 検査項目 | ルール |
|---|---|
| 設問数 | 8問であること |
| 四択 | 選択肢が4つ・正解インデックスが0〜3 |
| ○× | answerが true か false |
| 解説 | 全設問にあること |
| 禁止語 | フォーマル度・ドレスコード・ランク・レベル を含まない |

呉服の内容の正しさ・再登場の確認は **制作者が目視**で行う。

---

## 現在の状態

| 項目 | 状態 |
|---|---|
| Next.js プロジェクト | ✅ 作成済み |
| 3ペインUI | ✅ 実装済み |
| Claude API 生成 | ✅ 実装済み・動作確認済み |
| 機械検査 | ✅ 実装済み |
| 設問編集 | ✅ 実装済み |
| 承認（これでよい） | ✅ 実装済み |
| プレビュー | ✅ 実装済み |
| Quiz Studio 参照スキル | ✅ 作成済み（`.cursor/skills/quiz-studio-reference/SKILL.md`） |
| DB移行（SQLite + Drizzle） | ✅ 完了（`data/app.db`、`lib/db/`、`scripts/seed.ts`） |
| 問題作成スキル（Cursor用） | ✅ 作成済み（`create-lesson-questions`） |
| ガイド作成・レビュースキル群 | ✅ 作成済み（`create/review-series/course-guide` 計4本） |
| SeriesGuide スキーマ改訂 | ✅ 完了（`target`削除・`dan_definition`→`aux_concept`・7ファイル更新） |
| シリーズ/コースガイドの中身 | ❌ 空欄のまま（スキーマ改訂のみ。内容は次のチャットで記入） |
| スクリーンショット | ❌ まだ |
| 図解（提出物） | ❌ まだ |

---

## 次のチャットでやること（優先順）

1. ~~**DB移行（SQLite + Drizzle ORM）**~~ ✅ 完了

2. ~~**問題作成スキルの設計・作成**~~ ✅ 完了（`create-lesson-questions`）

3. ~~**ガイド作成・レビュースキル群の設計・作成**~~ ✅ 完了（計4本）

4. ~~**SeriesGuide スキーマ改訂**~~ ✅ 完了（`target`削除・`aux_concept`へ改名）

5. **シリーズガイドの作成**
   - 事前に `pnpm seed`（DB を最新 JSON に同期）→ `pnpm dev`
   - 新チャットで `create-series-guide` スキルを呼ぶ
   - 想定シリーズ（順不同）: 長襦袢・帯・ゆかた・七五三・振袖・男の着物・素材
   - 直近: 「格とTPO」完成 → 将来「長襦袢」へ

6. **コースガイドの作成**（`create-course-guide` スキルを呼ぶ）

7. 必要なら **ガイドのレビュー**（`review-series/course-guide`）

8. **レッスン2・3の生成**（`create-lesson-questions` スキルを呼ぶ）

9. **UI改善**（機能追加に合わせて相談しながら進める）

10. **LLM のアプリ内実装**（Agents Bar、最後に実施）

11. **以降は制作者が単独で実施**
    - 画面キャプチャ
    - 図解作成・提出

---

## 未決事項

- ~~問題作成スキルの具体的なステップ設計~~ ✅ 完了
- ガイドに書く呉服の正本の中身（用語・例外の具体的テキスト）
- `writing_style` の全シリーズ共通化可否（今セッション未詰め）
- Quiz Studio の 6本柱の具体的な中身（参照スキルに記載なし・照合未完了）
- 図解の提出形式・提出先（講師から指定があるはず）
- Anthropic APIキーは取得済み

---

## 既知の制約・注意事項

- **APIキーはページを閉じると消える**（ファイル保存なし。意図的な設計）
- **スクリーンショット時は DevTools を閉じる**（ネットワークタブにキーが見える）
- **モデル名**: `claude-opus-4-5`（`lib/prompt.ts` の `MODEL` 定数で変更可）
- **レッスン2の生成ロック**: レッスン1が「これでよい」になるまで生成ボタンは押せない
- **現在のブランチ**: `feature/beyond-mvp`（提出時は `git checkout main` で戻る）
- **DBはgit管理外**: `data/app.db` はブランチをまたいで共有される（コンテンツはどのブランチでも同じ）

---

## 後日追加予定（MVP外）

- P4 AI壁打ちチャット（ガイド内容や設問について Claude に相談）
- 設問タイプ：手順（3〜4項目を正しい順に並べる）
- レスポンシブ対応
