...
- 参照テキストは事実（柄の位置、袖の長さ、既婚・未婚、代表品目）の根拠にする。レンズ・出題範囲・禁止語はガイドが正。参照の言い回し（フォーマル度、段の序列、相当する洋装、題材の置き場）を設問に写さない。「ドレスコード」は招待状やお客様の発言の引用と、解説で平服の意味を返すときに使ってよい。正解の名称にするな
